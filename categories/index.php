...
<body>
    <div class="container">
        <div style="float: right" class="mt-2">
            <a href="createCategory.php" class="btn btn-sm btn-outline-success">New</a> | 
        ...
        <table id="example" class="table table-bordered table-hover" style="width: 100%">
            ...
            <tbody>
                <?php $i = 1; foreach ($all as $category): ?>
                    <tr>
                        <td><?= $i ?></td>
                        <td><?= $category->name ?></td>
                        <td><?= $category->code ?></td>
                        <td>
                            <?php if($category->status): ?>
                                <a href="../_actions/categories/suspended.php?id=<?= $category->id ?>" class="btn btn-sm btn-outline-success">Active</a>
                            <?php else: ?>
                                <a href="../_actions/categories/unsuspend.php?id=<?= $category->id ?>" class="btn btn-sm btn-danger">Suspended</a>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if($auth->value > 1): ?>
